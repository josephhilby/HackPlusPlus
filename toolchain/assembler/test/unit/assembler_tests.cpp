...
static std::string get_line(const std::vector<unsigned char> &v, size_t line0,
...
static void fail_with_diff(const std::string &stem,
  ...
  FAIL() << "Mismatch: " << stem << " (line " << (line0 + 1) << ", bit "
         << (col0 + 1) << ")\n"
...
TEST(Golden, Max) { run_golden_case("Max"); }

TEST(Golden, MaxL) { run_golden_case("MaxL"); }

TEST(Golden, Rect) { run_golden_case("Rect"); }

TEST(Golden, RectL) { run_golden_case("RectL"); }

TEST(Golden, Pong) { run_golden_case("Pong"); }
