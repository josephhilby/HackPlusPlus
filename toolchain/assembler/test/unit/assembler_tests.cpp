...
static std::vector<unsigned char> read_all(const fs::path& p) {
    ...
    // Keep it minimal; if open fails, vector will be empty and test will fail later.
...
static void fail_with_diff(const std::string& stem,
    ...
    FAIL() << "Mismatch: " << stem
           << " at byte " << i
           << " (line " << (line0 + 1) << ", col " << (col0 + 1) << ")\n"
           << "actual line:   \"" << act_line << "\"\n"
           << "expected line: \"" << exp_line << "\"\n"
           << "actual byte=" << (i < actual.size() ? (int)actual[i] : -1)
           << " expected byte=" << (i < expected.size() ? (int)expected[i] : -1);
}
...
// One test per file (easy to filter/debug)
TEST(Golden, Add)   {
    run_golden_case("Add");
}
